...
class AppTableAdmin(admin.ModelAdmin):
    def icon_show(self,obj):
        if obj.icon:
            return '<img width=50 src="/media/%s"></img>' % obj.icon
        else:
            return ''
    ...
    fields=['name','icon','icon2','file','link','content','state','app_class','channel',
            'version','cat','company','website','phone','email','must_update','hot']
