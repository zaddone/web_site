...
class NewEventImg(models.Model):
    ...
    def save(self,force_insert=False, force_update=False, using=None,
        ...
        if self.imgs:
            ...
            img_width, img_height = pixbuf.size
            max_width = 1080
            if img_width > max_width:
                _width, _height = scale_dimensions(img_width, img_height, max_width)
                pixbuf.thumbnail((_width, _height), Image.ANTIALIAS)
                pixbuf.save(self.imgs.path)
...
class NewDistrict(models.Model):
    district_id = models.IntegerField(u'旧城市id', default=0)
    district_name = models.CharField(u'城市名称',max_length=50, unique=True)
    title = models.CharField(u'城市拼音',max_length=30, blank=True,null=True,db_index=True)
    parent = models.ForeignKey('self', null=True, blank=True, related_name='children',verbose_name=u'上一级')
    #level = models.IntegerField()
    capital_letter = models.CharField(u'城市首字母',max_length=5, blank=True,null=True)
    usetype = models.IntegerField(blank=True, null=True, verbose_name=u'后台显示:1显示0不显示')
...
class NewVenueClass(models.Model):
    ...
    class Meta:
        ...
        verbose_name = u'地址场馆类别'
        verbose_name_plural = u'地址场馆类别' 
        # app_label = 'new_event'
...
class NewVenue(models.Model): 
    ...
    img = models.ForeignKey(NewEventImg,blank=True,null=True,verbose_name=u'图片')
    contact = models.SmallIntegerField(verbose_name=u'商务合作', default=0, choices=ct)
    # explore = models.SmallIntegerField(verbose_name=u'探店状态', default=0, choices=ex) #从explorer_info也可以曲折获得该信息
    phone = models.CharField(u'电话', max_length=100, blank=True)
    remark = models.CharField(u'备注', max_length=400, blank=True)
    data_from = models.SmallIntegerField(u'来源', default=1, choices=df_choices)
    tel_confirm = models.SmallIntegerField(u'电话确认', default=0, choices=tc_choices)
    create_time = models.DateTimeField(u'创建时间', auto_now_add=True)

    status = models.SmallIntegerField(u'小店状态', choices=st_choices, default=1)
    open_time = models.CharField(u'营业时间(请严格按照格式填写)', max_length=100, blank=True)
    venue_class = models.ManyToManyField(NewVenueClass,verbose_name=u'类型',blank=True,null=True)
...
class CityTheme(models.Model):
    ...
    keywords = models.CharField(max_length=10, blank=True)

    def __unicode__(self):
        return self.name

    class Meta:
        db_table = 'sys_city_theme'
        verbose_name = u'专题' 
        verbose_name_plural = u'专题'

class NewEventCat(models.Model): 
    st=((1,u'立减'),
              (2,u'折扣'),
              (3,u'限时'),
             )
    ...
    parent = models.ForeignKey('self', null=True, blank=True, related_name='children',verbose_name=u'上一级',)# models.ForeignKey('self',verbose_name=u'上一级', null=True, blank=True, related_name='children')
...
class NewEventFrom(models.Model):
    ...
    Website= models.CharField(u'网址',blank=True,null=True, max_length=200,  )
...
class NewEventTable(models.Model):
    ...
    theme = models.ManyToManyField(CityTheme, blank=True, null=True, verbose_name=u'专题')
    tag = models.ManyToManyField(NewEventTag,blank=True,null=True,verbose_name=u'标签')
    city= models.ManyToManyField(NewDistrict,blank=True,null=True,verbose_name=u'城市')
    addr=models.ManyToManyField(NewVenue,blank=True,null=True,verbose_name=u'地址场馆', related_name='addr_event')
    ...
    shoporevent = models.SmallIntegerField(u'小店/活动/网页', blank=True, choices=se_choices, null=True)
    ...
    def getFirstCity(self):
        return self.city.all()[0]
    
    def getAddress(self):
        res = ''
        if self.addr.all().count():
            addr_object = self.addr.all()[0]
            res = '%s,%s'%(addr_object.address,addr_object.title) 
        else:
            res = self.getFirstCity().district_name
        return res
    
    def getImageUrl(self):
        if not self.img.all().count():
            try:
                return 'http://pic.huodongjia.com/img/default%s.jpg'%self.getFirstCat().cat_id
            except:
                return 'http://pic.huodongjia.com/img/default4.jpg'
        img = self.img.all().order_by('order')[0]
        url = img.urls
        import re
        if not re.match('/.+',url):
            url = '/' + url
        
        return 'http://pic.huodongjia.com'+url
    
    def getFirstCat(self):
        res = 0
        q = self.cat.filter(cat_id__lt = 100)
        if q.count():
            res = q[0]
        return res
...
class NewEventPriceUnit(models.Model):
    ...
    event=models.ForeignKey(NewEventTable,related_name='Price_event_table',blank=True,null=True,verbose_name=u'关联活动')
    price=models.DecimalField(u'售价',max_digits=7, decimal_places=2)
    sale=models.DecimalField(u'折扣价格 ',max_digits=7, decimal_places=2,blank=True,null=True)
    discount=models.DecimalField(u'折扣率 ',max_digits=7, decimal_places=2,blank=True,null=True)
    original_price=models.DecimalField(u'反现',max_digits=7, decimal_places=2,blank=True,null=True)
    ...
    status = models.PositiveSmallIntegerField(u'有效状态',blank=True,null=True,default=1,choices=st )
    type=models.PositiveSmallIntegerField(u'价格类型',blank=True,null=True,default=0,choices=typ )
...
class NewOrder(models.Model):
    ...
    # order_payment = models.CharField(u'付款方式',max_length=100, blank=True)
    order_payment = models.SmallIntegerField(u'付款方式',default=0,choices=op_choices)
    ...
    admin_name = models.CharField(u'最后修改',max_length=100, blank=True)
    admin = models.ForeignKey(User,verbose_name=u'创建编辑',blank=True,null=True,related_name='order_edit')
    admin_text = models.CharField(u'管理备注',max_length=200, blank=True)
    order_telphone = models.CharField(u'固话',max_length=20, blank=True)
    order_pay_status = models.IntegerField(blank=True, null=True,verbose_name=u'处理状态',choices=ct)
    order_status = models.IntegerField(verbose_name=u'订单状态',blank=True, null=True,choices=st)
    event  = models.ForeignKey(NewEventTable,verbose_name=u'关联活动',blank=True,related_name='order_event')
    event_name = models.CharField(u'活动名称',max_length=200, blank=True)
    city_title = models.CharField(u'城市',max_length=10, blank=True)
    ...
    order_type = models.SmallIntegerField(u'支付/报名', default=0, choices=ot_choices)
...
class NewArticle(models.Model):
    ...
    '''
    name=models.CharField(u'标题',max_length=100)
    cat= models.OneToOneField(NewEventCat,blank=True,null=True,verbose_name=u'分类',related_name='ar_cat')
...
class VisitRecord(models.Model):
    event = models.OneToOneField(NewEventTable,related_name='event_visit_record',verbose_name=u'关联活动')
...
class PostEvent(models.Model):
    id = models.IntegerField(primary_key=True)
    host_tel=models.CharField(max_length=30, blank=True)
    host_mail = models.CharField(max_length=30, blank=True)
    event_url = models.CharField(max_length=255, blank=True)
    event_file_path = models.FileField(max_length=200, upload_to = 'host_doc',blank=True)
    event_priority = models.IntegerField(blank=True,)
    host_ip = models.CharField(max_length=15,blank=True)
...
class feelnum(models.Model):
    ...
    feelnum=models.IntegerField(u'数据',blank=True,null=True)
...
class ImageAds(models.Model):
    event = models.ForeignKey(NewEventTable, verbose_name='关联活动')
    city = models.ManyToManyField(NewDistrict, verbose_name='投放城市')
    img = models.ForeignKey(NewEventImg, verbose_name='图片', related_name='ads')
    ...
    class Meta:
        db_table = 'sys_app_ads'
        verbose_name =  u'广告'
        verbose_name_plural = u'广告'

class HotWords(models.Model):
    words = models.CharField(u'关键词(中英逗号分隔都行)', max_length=100)
    city = models.OneToOneField(NewDistrict, verbose_name=u'城市')
